...
module.exports = Ramdani = async (Ramdani, mek) => {
	try {
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = Ramdani.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","footerText": "© BY KEPIN CUY","listType": "SINGLE_SELECT","sections": list}}, {})
...
"externalAdReply": {
     ...
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
       ...
       console.error(err)
        ...
        const ftoko = {key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync(`./media/foto/thumb.jpg`)},"title": `© BY KEPIN CUY`,"description": "Ramdani Official", "currencyCode": "IDR","priceAmount1000": "999999","retailerId": "Ramdani Official","productImageCount": 1},"businessOwnerJid": `0@s.whatsapp.net`}}}
...
danzrun = process.uptime() 
           Ramdani.setStatus(`${namabot} Aktif Selama ${(danzrun)} © Creator By Kepin Ges`).catch((_)=>_); //DI UBAH YATIMM
...
> 532 ${prefix}nsfw_avatar`
               ...
               buttonsMessage = {
               contentText: `${allmenu}`,
               footerText: '© BY KEPIN CUY', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
//------------------<18+ Menu>-----------------------   
       case 'randombokep':
              ...
              Ramdani.sendMessage(from, '*PERMINTAAN:* '+bokep+'\n*DOSA TANGGUNG PRIBADI*\n*NI BRO FREE BUAT KAMU DOWNLOAD SENDIRI:* '+ kep, text, { quoted: ftoko, caption: `NI BOKEP SAYA DAPAT DARI *© BY KEPIN CUY* DOSA TANGGUNG SENDIRI🗿`})
              break
                case 'xnxx':
              if (!isRegist) return freply(mess.regist)
              if (isDewasa) return freply(mess.dewasa)
